...
const BloodBankProfile=()=>{
    ...
    const deleteBloodbank=(user_id,e)=>{
      ...
      axios.delete(`http://localhost:8080/deleteBloodbank?user_id=${user_id}`)
    ...
    const sendData=(id,e)=>
    {
        e.preventDefault();
        console.log("hi");
      axios.put(`http://localhost:8080/updateBloodbank?bloodbank_id=${id}`,{
               ...
               {/* <Multiimage></Multiimage> */}
                ...
                <div class="col-sm-8 mt-3" id="signupForm">
                  ...
                  <div className="form" >
                    ...
                    <input name="bloodbank_name" type="text " className="form-control" placeholder="Enter Blood Bank Name" 
                    value={reg.bloodbank_name} onChange={(e)=>{dispatch({type:'update',field:'bloodbank_name',val:e.target.value})}} required />
                    </div>
                    ...
                    <button  className="btn btn-md btn-block btn-primary fw-bold" style={{ marginRight: "10px" }}
                       type="submit" onClick={bloodbankhome}>
                        Goto Home
                      </button><span></span> <span> </span>
                      <button  className="btn btn-md btn-block btn-success fw-bold" style={{ marginRight: "10px" , marginLeft:"10px" }}
                       type="submit" onClick={(e)=>{sendData(reg.id,e)}}>
                        Update BloodBank
                      </button><span></span> <span> </span>
                      <button className="btn btn-md btn-block btn-danger fw-bold " style={{ marginLeft: "10px" }}
                      onClick={(e)=>{deleteBloodbank(reg.user_id,e)}}>Delete BloodBank</button>
                    </div>
                    {msg}
                  </div>
                </div>
