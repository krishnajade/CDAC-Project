...
const BloodBankStock = () => {
  ...
  useEffect(() => {
    fetch(
      `http://localhost:8080/allBloodStockById?bloodbank_id=${sessionStorage["bloodbank_id"]}`
    )
      .then((resp) => resp.json())
  ...
  const sendData = (id) => {
    ...
    axios
      .put(
        `http://localhost:8080/updatebloodstock?bloodstock_id=${id}&quantity=${qnt}`,
        {}
      )
      .then((response) => {
