...
const AcceptedHospRequest=()=>{

    const[request,setRequest]=useState([]);
    const[id,setId]=useState(0);

    useEffect(()=>{
        fetch(`http://localhost:8080/Hospitalrequestbystatus?hospital_id=${sessionStorage["hospital_id"]}&request_status=accepted`)
        .then(resp=> resp.json())
...
export default AcceptedHospRequest;